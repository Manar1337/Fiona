class_name LeftWalls
extends Node2D

@onready var wall1: Wall = $LeftWall1
@onready var wall2: Wall = $LeftWall2
@onready var wall3: Wall = $LeftWall3
...
func _ready() -> void:
	wall1.set_type(WallPolygon.WallPolygonType.LEFT)
	wall2.set_type(WallPolygon.WallPolygonType.LEFT)
	wall3.set_type(WallPolygon.WallPolygonType.LEFT)
...
func change_wall(wall:Wall, wall_type:WallComponent) -> Wall:
	return wall_type.change_left_wall(wall)
