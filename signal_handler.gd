...
signal show_escape_message(show_message: bool)
signal show_high_score(onoff: bool)
signal show_gui(will_show_gui: bool)
signal player_sent_to_hell(current_level: int)
signal death_tunnel_progress_changed(distance_left: int, distance_total: int, tunnel_speed: float)
signal escaped_tunnel()
signal game_over()
signal restart_game()
